...
impl Api {
    pub fn new(options: Option<ApiOptions>, board_id: &str, section_id: &str) -> Self {
        Self {
            options,
...
mod tests {
    ...
    // BEARER_CODE=xxx BOARD_ID=xxx SECTION_ID=xxx cargo test test_get_boards_board_sections_section_id_pins -- --nocapture --test-threads=1

    #[tokio::test]
    async fn test_get_boards_board_sections_section_id_pins() {
        let bearer_code = std::env::var("BEARER_CODE").unwrap_or_default();
        let board_id = std::env::var("BOARD_ID").unwrap_or_default();
        let section_id = std::env::var("SECTION_ID").unwrap_or_default();
        let response = Api::new(None, &board_id, &section_id)
